...
select account_id,
 ...
 Floor(sum( if( type = 'Outgoing', amount, 0 ))) AS Outgoing,
...
select date,count(*) from loan where LEFT(CAST(date as CHAR(50)),4)="9712" group by date,duration having count(*)>0;
...
select account_id,type as transaction_type, Floor(sum(amount)) as total_amount from trans where account_id=396 group by type order by type;
